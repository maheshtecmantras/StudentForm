...
def feedback_form():
    ...
    if request.method == 'POST':
        ...
        tech_name = candidate['technology_name']  # technology name
        decision, description = evaluate_candidate(tech_name, review)
        print(description,decision)
        # Update the candidate record
        cursor.execute("""
            UPDATE candidates
            SET review = %s, description = %s, decision = %s
            WHERE id = %s
        ...
        # Send mail to faculty for this candidate
        send_mail_to_hr(candidate_id)
